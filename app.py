    ...
    if not target_file or not seed1_file:
        ...
        with st.spinner('처리 중...'):
            try:
                ...
                if df_seeds.empty:
                    st.error("데이터를 찾지 못했습니다. 엑셀 파일 양식을 확인해주세요.")
                    st.stop()
                ...
                st.download_button(
                    label="📥 다운로드", 
                    data=out, 
                    file_name=f"{datetime.now().strftime('%Y%m%d')}_{target_file.name}", 
